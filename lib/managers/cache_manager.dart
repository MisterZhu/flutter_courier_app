...
/// global：存放的键值对保存在内存中，存取方便，但是会占用内存
/// local：存放的键值对存在本地数据库的，需要的时候再查找，需要异步查找
...
class CacheManager {
  ...
  Future<void> init() async {
    ...
    _encryptedBox = await Hive.openBox(_encryptName, encryptionCipher: HiveAesCipher(encryptionKey));
  ...
  putGlobal({dynamic key, dynamic value}) async => await _globalBox.put(key, value);

  containsGlobal(dynamic key) => _globalBox.containsKey(key);

  getGlobal(dynamic key, {dynamic defaultValue}) => _globalBox.get(key, defaultValue: defaultValue);

  deleteGlobal(dynamic key) async => _globalBox.delete(key);

  putLocal({dynamic key, dynamic value}) async => _localBox.put(key, value);

  getLocal(dynamic key, {dynamic defaultValue}) async => _localBox.get(key, defaultValue: defaultValue);

  deleteLocal(dynamic key) async => _localBox.delete(key);

  deleteLocals(List keys) async => _localBox.deleteAll(keys);

  putEncrypt(dynamic key, dynamic value) async => await _encryptedBox.put(key, value);

  getEncrypt(dynamic key, {dynamic defaultValue}) => _encryptedBox.get(key, defaultValue: defaultValue);
